...
def a_func(x):
    ...
    if a_choice == 1:
        ...
        return 1.0+alpha*numpy.cos(2.0*numpy.pi*x)##0.25 - 0.15*float(0.25 < x < 0.5)
    else:
...
if implicit:
    ...
    for i in range(0,Nx):
        ...
        data.append( a[numpy.mod(i+1,Nx)]*dt_over_h/2.0 )
...
for nt in range(0,Nt):               
    if t_choice == 1:
        ...
        for i in range(0,Nx):        
            next_u[i] = -dt_over_h/2*(u[i]-u[i-1])+u[i]  # (exemple)
    else:
